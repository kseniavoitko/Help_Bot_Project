...
class Record:
    def __init__(self, number: str, date_create: str, name: Title, description: Description = None, tag: Tags = None, deadline = None, state = bool) -> None:
        self.number = number
    ...
    def days_to_deadline(self):
        if self.deadline:
            ...
            if day_now > day_dn:
                day_dn = datetime(day=self.deadline.value.day, month=self.deadline.value.month, year=datetime.now().year+1).date()
            difference = day_dn-day_now
            return difference.days if difference.days>=0 else ""
        return ""

    def __str__(self) -> str:
        return "{:>6} {:^12} {:<18} {:<60} {:<12} {:^12} {:^6} {:^6}".format(str(self.number), str(self.data_create), str(self.name), str(self.description), 
                                                                         ', '.join(str(p) for p in self.tags), str(self.deadline), '+' if self.state == True else '',str(self.days_to_deadline()))
